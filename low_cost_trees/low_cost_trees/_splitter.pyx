...
cdef class WeightedRandomSplitter(BaseDenseSplitter):
    cdef object feature_weight
    cdef object current_weight
    cdef object name_map
    cdef object weight_map
    cdef object weight_used
    ...
    def add_weight(self, object feature_weight, object name_map,
                                object weight_map):
    ...
    cdef SIZE_t weighted_random_index(self, SIZE_t low, SIZE_t high) except -1: #low=n_drawn_constants, high=f_i-n_found_constants
        """Generate a random integer in [low; end)."""
        weight_sum = 0
        curr_feature_weight = []

        min_weight = 10000
        for f in self.feature_weight:
            if self.feature_weight[f] < min_weight and self.feature_weight[f] > 0:
                min_weight = self.feature_weight[f]

        for i in range(low, high):#self.feature_weight.shape[0]):
            curr_feature = self.name_map[self.features[i]]
            curr_weight = 0

            if curr_feature not in self.weight_used:
                curr_weight = 0 #self.feature_weight[curr_feature]
                seen = set()
                to_add = set([curr_feature])
                next = curr_feature
                while next not in seen:
                    seen.add(next)
                    for subweight in self.weight_map[next]:
                        to_add.add(subweight)
                    if seen != to_add:
                        next = list(to_add - seen)[0]

                for subweight in to_add:
                    if subweight not in self.weight_used:
                        curr_weight += self.feature_weight[subweight]

            if curr_weight == 0: #after having added the weights of all dependencies, this feature still has zero weight
                curr_weight = min(min_weight, 1) #will use the inverse, so need a low number (but not zero)

            curr_weight = 1.0 / curr_weight
            curr_feature_weight.append(curr_weight)
            weight_sum += curr_weight
        rnd = random.random() * weight_sum
        for i in range(len(curr_feature_weight)):
            rnd -= curr_feature_weight[i]
            if rnd < 0:
                return i + low
        return high - 1
    ...
    cdef int node_split(self, double impurity, SplitRecord* split,
        ...
        while (f_i > n_total_constants and  # Stop early if remaining features
            ...
            #               random_state)
            with gil:
                f_j = self.weighted_random_index(n_drawn_constants, f_i - n_found_constants)
        ...
        # update the feature weights information
        with gil:
            curr_feature = self.name_map[best.feature]
            if curr_feature not in self.weight_used:
                self.weight_used.append(curr_feature)
            for subweight in self.weight_map[curr_feature]:
                if subweight not in self.weight_used:
                    self.weight_used.append(subweight)
